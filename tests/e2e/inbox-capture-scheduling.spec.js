...
test("Today Inbox converts once and Task drag placement commits immediately", async ({ page, request }) => {
  ...
  await expect(inbox.locator(".today-inbox-count")).toHaveCSS("color", "rgb(0, 0, 0)");
  ...
  await completeCaptureRelations(projectCapture, 1);
  ...
  const placement = await page.evaluate(() => {
    const task = createTask("즉시 사라질 미계획 Task", {
      deferCreate: true,
      initial: { boxId: "", projectId: "", resourceId: "", dueDate: "", status: "todo" },
    });
    saveState();
