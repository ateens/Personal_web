# Resource / Notion parity final local gap audit

Date: 2026-07-11 (Asia/Seoul)

Branch: `codex/resource-notion-parity-phase-a`

Audit point: local implementation and automated verification complete enough for a deployment gate; production activation is not complete
...
- `[Release-blocking]` The public production trust boundary is not activated. The 2026-07-11 live audit still observed anonymous `200` responses from the public Sites and Railway state endpoints, while the production DB policy reported `configured:false` and `enforced:false`. The branch contains the staged bearer implementation and runbook, but neither the private Sites Worker nor Railway enforcement has been deployed and verified as a pair.
- `[Verified]` Phase B's local core is implemented: IndexedDB snapshot/operation stores, replay/retry/conflict behavior, truthful save states, Resource-level incremental writes, waiting service-worker update gating, migration snapshots, integrity-checked restore, and isolated database checks.
...
| P0 · Resolved locally | Resource v4 model, timestamp, revision | `app.js` `normalizeResourceRecord`, `touchResource`; `server/storage.js`; `check-postgres-state.mjs` | `createdAt`, `updatedAt`, `revision`, `timestampSource`, page fields를 migration한다. mutation만 timestamp/revision을 올리고 open/scroll은 올리지 않으며 updated sort에 안정적 tie-break를 사용한다. | page 생성/수정 metadata와 실제 최근 수정 정렬이 필요하다. | legacy timestamp는 migration 시각이며 과거 실제 수정 시각으로 주장하지 않는다. | migration source label을 유지하고 production pre-deploy backup을 만든다. | mutation/non-mutation E2E와 isolated PostgreSQL migration check 통과. | `[Verified]` |
...
| P0 · Resolved locally | Migration backup/restore | `server/storage.js`; `scripts/manage-state-backups.mjs`; `check-state-migration-backups.mjs`; backup runbook | automatic migration/read-heal 전에 full authoritative snapshot을 같은 transaction에 저장한다. Manual create/list/restore는 workspace scope, SHA-256, revision precondition, safety backup, monotonic revision, restore history를 사용한다. | migration 실패와 operator error에서 복구 가능한 checkpoint가 필요하다. | 같은 PostgreSQL 안의 backup이므로 DB/account loss는 막지 못하고 retention/prune도 의도적으로 없다. | production pre-deploy snapshot과 별도 encrypted PITR/dump를 운영한다. | `npm run check:backups` isolated v3→v4/create/list/restore/sentinel cleanup 통과. | `[Verified]`; production backup 실행 전 |
| P0 · Release-blocking | 공개 API 인증과 workspace boundary | `server.js`; `worker/index.js`; `configure-api-proxy-auth.mjs`; API auth runbook | branch는 DB-backed staged bearer gate, fail-closed override, Worker header stripping/injection, authenticated Sites identity requirement를 구현한다. 현재 모델은 하나의 `APP_STATE_ID`를 공유하는 single-workspace gate다. | 외부 노출 state API는 authenticated boundary 뒤에 있어야 한다. | production Sites/Railway state endpoints는 아직 anonymous `200`; per-user/role/tenant authorization도 없다. | 코드 배포 → credential stage → private Sites secret/gate 검증 → DB enforcement enable → direct Railway missing/wrong `401`, private Sites `200` 검증 순서를 지킨다. | local `check:api-auth`/Worker checks 통과. 2026-07-11 production recheck는 미활성 상태를 확인했다. | local `[Verified]`; production activation `Pending` |
...
| P1 · Release gate | Deployment, production migration, visual hand-off | branch, runbooks, `.openai/hosting.json` | validated source and runbooks exist locally. No production code/auth migration or Sites release is claimed. | deployed candidate must match tested source and protect state before external use. | production backup, backend rollout, private Sites save/deploy, auth activation, and post-deploy smoke tests remain. | follow the ordered migration/auth gates below and archive exact revision + screenshots. | local checks only. | production `Pending` |
...
Do not enable database enforcement before the private Sites path has the matching secret and is proven to work.

1. Run the complete checks against the exact commit to deploy.
2. Create a production metadata-only manual checkpoint with `scripts/manage-state-backups.mjs create --reason pre_deploy` and separately confirm PostgreSQL PITR/dump coverage.
3. Deploy the fail-closed-capable server and Worker code with DB enforcement still disabled.
4. Stage a credential via a mode-`0600` token file. Never print or pass the token in command arguments.
5. Make Sites private, set `REQUIRE_AUTHENTICATED_PROXY=1`, install the same value as the Sites `API_BEARER_TOKEN` secret, deploy, and verify signed-in access while DB enforcement is still off.
6. Enable the DB-backed policy, wait for the policy cache or invalidate it safely, then verify direct Railway missing/wrong credentials return `401` and the private Sites path still returns `200`.
7. Record the commit, deployment IDs, auth fingerprint, backup ID, workspace revision, and post-deploy checks without recording secrets or state.
...
This branch is a broad local implementation checkpoint, not a completed production rollout and not a claim of visual identity with Notion. The remaining P0 is the production authentication/deployment gate. Authenticated Notion screenshot comparison, real mobile soft-keyboard/screen-reader QA, and the listed P1/P2 product capabilities remain explicitly open.
